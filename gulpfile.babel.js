...
export function imgSandbox() {
  return gulp.src(paths.imgSandbox.src).pipe(gulp.dest(paths.imgSandbox.dest));
}
...
export function release(callback) {
  return gulp.series(version, commit, push, tag, authenticate, function(error) {
    if (error) {
...
const pathsApp = {
  colors: {
    src: "assets/scss/common/color.scss",
    file: "colors.js",
    dest: BASEPATH_APP
  },
  fonticon: {
    src: ["assets/svg/**.svg", "assets/svg/social/**.svg"],
    template: "assets/scss/common/_fonticon-mobile.scss.template",
    tmp: "fonticon-tmp.js",
    dest: BASEPATH_APP
  }
};
...
export function scssToJsonColors() {
  const fileColors = scssToJson(pathsApp.colors.src);
  let colors = JSON.stringify(fileColors);
  colors = colors.replace(/\$color-/g, ""); // suppression des $color-
  colors = colors.replace(/-*(\w)(\w*)/g, (match, p1, p2) => {
    // ajout des majuscules et suppression des tirets
    return p1.toUpperCase() + (p2 || "");
  });

  return $.file(
    pathsApp.colors.file,
    `const Colors = ${colors}\nexport default Colors`,
    { src: true }
  ).pipe(gulp.dest(pathsApp.colors.dest));
}

export function createFonticon() {
  return gulp
    .src(pathsApp.fonticon.src)
    .pipe(
      $.iconfont({
        fontName: "Lib-Nickel-Icon",
        prependUnicode: false,
    ...
    .on("glyphs", function(glyphs) {
      gulp
        .src(pathsApp.fonticon.template)
        .pipe(
          $.consolidate("lodash", {
            glyphs,
            fontName: "Lib-Nickel-Icon",
        ...
        .pipe($.rename(pathsApp.fonticon.tmp))
        .pipe(gulp.dest(pathsApp.fonticon.dest));
    })
    .pipe(gulp.dest(paths.fonticon.dest));
}
...
export function scssToJsonFonticon() {
  setTimeout(function() {
    let fonticon = fs.readFileSync(
      `${BASEPATH_APP}${pathsApp.fonticon.tmp}`,
      "utf8"
    );
...
export function generateJsToScss() {
  gulp.series(createFonticon, scssToJsonFonticon);
}
gulp.task("generate-app-fonticon", createFonticon);
